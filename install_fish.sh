...
#   3. Python package (GPU nsys, CPU perf, daemon, logging)
#   4. Settings file (fish_settings.ini — shared config for all components)
#
# Output structure per session:
#   /tmp/fish_traces/<session>/
#     ├── ros2/       — LTTng trace data
#     ├── nsys/       — Nsight Systems reports
#     ├── perf/       — perf call-graph data
...
# perf requires paranoid level <= 0 for process-tree profiling
if [[ -f /proc/sys/kernel/perf_event_paranoid ]]; then
    ...
    if [[ "$CURRENT_PARANOID" -gt 0 ]]; then
        ...
        if [[ $? -eq 0 ]]; then
            echo "[FISH] Set perf_event_paranoid to -1 (was $CURRENT_PARANOID)"
        else
            echo "[FISH] WARNING: Could not set perf_event_paranoid (need root). perf may not work."
        fi
    fi
fi
...
if [[ -d "$SCRIPT_DIR/python/fish" ]]; then
    ...
    echo "[FISH] WARNING: python/fish/ not found, GPU/perf features unavailable"
fi
...
#!/bin/bash
# FISH trace session management (LTTng + daemon)
...
start_session() {
    ...
    if [[ -z "\$SESSION" ]] || ! lttng list "\$SESSION" &>/dev/null; then
        ...
        # Auto-start daemon (handles both GPU nsys and CPU perf)
        PYTHONPATH=\$FISH_PYTHON:\$PYTHONPATH python3 -m fish.cli gpu daemon start 2>/dev/null
    fi
}

stop_session() {
    # Stop daemon first (flushes nsys reports and perf data)
...
echo "  ros2 run fish gpu nodes       # list all ROS 2 nodes"
