...
# Issue 3: isaac_ros_nitros_bridge_ros2 (transitively pulled in by pynitros)
#   includes <boost/uuid/uuid.hpp> but the base only has libboost-python1.74.0,
#   no dev headers. Install libboost-dev.
...
patch_pre_build() {
    ...
    for ft in "$SRC_FT" "$INST_FT"; do
        if [ -f "$ft" ] && ! grep -q TRT_MAJOR_ENTERPRISE "$ft"; then
            ...
            ' "$ft"
            echo "[patch:pynitros] patched $ft for TensorRT 10.x"
        fi
    done
