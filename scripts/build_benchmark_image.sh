...
# Build the clone-loop command from the workload_repos array
CLONE_CMDS=""
for repo in $WORKLOAD_REPOS; do
    CLONE_CMDS="${CLONE_CMDS}              git clone --depth 1 --branch release-3.2 https://github.com/NVIDIA-ISAAC-ROS/${repo}.git || true ; \\\\
"
done
# colcon should target the benchmark package itself (it picks up its declared deps)
COLCON_TARGETS="${BENCHMARK}_benchmark isaac_ros_benchmark"
...
# Run the GPU mode of the setup script + clone the workload repos on top.
...
${CLONE_CMDS}\
              ...
              colcon build --packages-up-to ${COLCON_TARGETS} 2>&1 | tail -25" && \\
