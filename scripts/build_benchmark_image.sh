...
# FROM fish-r2b-base means common Isaac ROS workspace (ros2_benchmark,
# isaac_ros_common, isaac_ros_benchmark, isaac_ros_apriltag, image_pipeline,
# vision_opencv/image_geometry) is already built. Per-benchmark build only
# needs to clone the workload repo + colcon build the new package(s).
cat > "$DOCKERFILE" <<EOF
