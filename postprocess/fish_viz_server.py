...
def _namespace_views(fnodes, edges, min_f=2, neighbors='show', isolated='hide', depth=1, infra_mode='exclude'):
    ...
    for ns, fids in sorted(by_ns.items()):
        ...
        if isolated == 'hide' and v_edges:
            ...
            # "hide dead sinks / leaf topics"); keep the view if that empties it
            deg = set()
            for e in v_edges: deg.add(e['src']); deg.add(e['dst'])
            kept = [n for n in v_nodes if n['id'] in deg]
            if kept:
                v_nodes = kept
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif path in ('/f6', '/f6.html', '/f6_dag.html'):
            _serve_file(self, os.path.join(HERE, 'f6_dag.html'), 'text/html; charset=utf-8')
